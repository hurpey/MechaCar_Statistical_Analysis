...
The metrics which would be of interest to customers and competitions comparable model are as follows:

1. Fuel efficiency (MPG)
2. Sales Price of the Vehicle
3. Maintainance cost of the Vehicle

Fuel efficiency: Fuel efficiency relates to a vehicles gas consumption while in use. More consumers tend towards fuel-efficient cars due to increased cost savings. In measuring whether MechaCars products have a better fuel efficency compared to competitors, we can perform ANOVA tests. A one-way ANOVA is used to test the means of a single dependent variable across a single independent variable with multiple groups(e.g.fuel efficiency(mpg) of different cars based on vehicle class).For conducting this test, mpg data of all the concerned manufacturers are required. The Null hypothesis is that the means of mpg of all groups are equal. The Alternate hypothesis is that at least one of the means is different from all other groups.

Sales Price of the Vehicle: The price of a vehicle is important for many consumers. Consumers will compare the price of a vehilce with added features and further compare this across various models and brands. We can use the multiple linear regression to determine the factors that have the highest correlation/predictability with the list sales price (dependent variable) and which combination has the greatest impact on price. The Null Hypothesis is that MechaCar is priced correctly based on its performance of key factors for its genre. The Alternate Hypothesis is that MechaCar is NOT priced correctly based on performance of key factors for its genre. 

Maintenance cost of the Vehicle: This will involve comparing various maintenance costs such as  cost of repairs, insurance, rate of depreciation and "second-hand value" of the Mechacar with other competitors.  To compare the cost of various manufacturers we need to perform ANOVA test in which we need to have mainly cost and fuel efficiency data of all the concerned manufacturers.The Null hypothesis will be that the means of costs of all groups are equal. The Alternate hypothesis is that at least one of the means is different from all other groups. The same test can be performed for reliability of cars between various car manufacturers.
